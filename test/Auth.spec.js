...
describe('Test para userState', () => {
    it('userState debería ser una funcion', () => {
        expect(typeof userState).toBe('function')
    });
    it('userState debería llamar a onAuthStateChanged con sus parámetros', () => {
        userState('user');
        expect(onAuthStateChanged).toHaveBeenCalledWith(auth, 'user')
    })
});

describe('Test para user1', () => {
    it('user1 debería ser una funcion', () => {
        expect(typeof user1).toBe('function')
    });
    it('user1 debería retornar a currentUser', () => {
        const user = { uid: 'user', email: 'email@user.com' };
